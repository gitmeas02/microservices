@echo off
REM Docker-based SSL Certificate Generation Script
...
echo.
echo ========================================
echo  Docker SSL Certificate Generator
echo ========================================
echo.
...
REM Create certificate configuration file
set "CONFIG_FILE=%SSL_DIR%\openssl.conf"
(
echo [req]
...
echo 📝 Created certificate configuration file

echo 🔑 Generating private key using Docker...
docker run --rm -v "%SSL_DIR%:/ssl" frapsoft/openssl genrsa -out /ssl/dev-key.pem 2048
if !errorlevel! neq 0 (
...
echo 📜 Generating certificate using Docker...
docker run --rm -v "%SSL_DIR%:/ssl" frapsoft/openssl req -new -x509 -key /ssl/dev-key.pem -out /ssl/dev-cert.pem -days 365 -config /ssl/openssl.conf -extensions v3_req
if !errorlevel! neq 0 (
...
del "%SSL_DIR%\openssl.conf" 2>nul
...
REM Show certificate details using Docker
echo ⏱️  Certificate Details:
docker run --rm -v "%SSL_DIR%:/certs" alpine/openssl x509 -in /certs/dev-cert.pem -text -noout | findstr /C:"Not Before" /C:"Not After" /C:"Subject:" /C:"DNS:" 2>nul
echo.
